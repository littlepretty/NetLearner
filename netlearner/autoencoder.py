...
class Autoencoder(object):
    def __init__(self, feature_size, encode_size, transfer_func=tf.nn.softplus,
                 optimizer=tf.train.AdamOptimizer,
                 sparsity=0.0, sparsity_weight=0.0, mask_fraction=0.0, name='AE'):
        ...
        self.encode_lr = tf.placeholder(tf.float32, name='learning_rate')
        ...
        self.kl_divergence_record = tf.placeholder(tf.float32, name='kl_divergence')
        ...
        self.optimizer = optimizer(self.encode_lr).minimize(self.loss)
    ...
    def _initialize_weights(self):
        all_weights = dict()
        all_weights['w1'] = tf.Variable(tf.random_normal([self.feature_size, self.encode_size],
                                                         mean=0.0, stddev=0.01))
        all_weights['b1'] = tf.Variable(tf.zeros([self.encode_size], dtype=tf.float32))

        all_weights['w2'] = tf.Variable(tf.random_normal([self.encode_size, self.feature_size],
                                                         mean=0.0, stddev=0.01))
        all_weights['b2'] = tf.Variable(tf.zeros([self.feature_size], dtype=tf.float32))
        return all_weights

    def _create_encode_node(self):
        logits = tf.add(tf.matmul(self.x, self.weights['w1']), self.weights['b1'])
    ...
    def _create_kl_node(self):
        """Override me if dataset is binary and cost is measured by KL-divergence"""
        return None
    ...
    def _create_summaries(self):
        ...
        tf.summary.scalar('learning rate', self.encode_lr)
    ...
    def train_with_labels(self, train_dataset, train_labels, batch_size, num_steps,
        ...
        for step in range(num_steps):
            ...
            lr = init_lr * np.power(0.6, step / num_steps)
            loss = self.partial_fit(batch_data, lr)
            ...
            if step != 0 and step % summary_step == 0:
                ...
                summaries = self.sess.run(self.merged_summary,
                                          feed_dict={self.x: batch_data,
                                                     ...
                                                     self.encode_lr: lr,
                                                     self.keep_prob: 1 - self.mask_fraction})
            ...
            if step != 0 and step % display_step == 0:
                ...
                train_loss = self.calc_reconstruct_loss(train_dataset)
                valid_loss = self.calc_reconstruct_loss(valid_dataset)
                print("Minibatch(%d cases) loss at step %d: %.6f(kl=%.4f, lr=%.6f)"
...
class SparseAutoencoder(Autoencoder):
    def __init__(self, feature_size, encode_size, transfer_func=tf.nn.sigmoid,
                 optimizer=tf.train.AdamOptimizer, sparsity=0.01, sparsity_weight=0.05,
                 name='SparseAE'):
        super(SparseAutoencoder, self).__init__(
            feature_size, encode_size, transfer_func, optimizer,
            sparsity=sparsity, sparsity_weight=sparsity_weight, name=name)

    def _create_kl_node(self):
        """Calculate the kl divergence between active nodes and sparsity vector"""
        # return tf.reduce_sum(tf.reduce_mean(tf.abs(self.encode), reduction_indices=0))
...
class MaskNoiseAutoencoder(Autoencoder):
    def __init__(self, feature_size, encode_size, transfer_func=tf.nn.sigmoid,
                 optimizer=tf.train.AdamOptimizer, mask_fraction=0.4, name='MaskNoiseAE'):
    ...
    def calc_reconstruct_loss(self, X):
        return self.sess.run(self.loss, feed_dict={self.x: X, self.keep_prob: 1.0})

    def encode_dataset(self, X):
        return self.sess.run(self.encode, feed_dict={self.x: X, self.keep_prob: 1.0})
