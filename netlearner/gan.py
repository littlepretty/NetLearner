...
import numpy as np
from utils import next_batch, plot_samples, plot_V
    ...
    def train(self, batch_size, train_dataset, num_steps, init_lr=0.001):
        ...
        for step in xrange(num_steps):
            ...
            if step % display_step == 0:
                ...
                plt.savefig('%s/mnist_sample_%d.png' % (self.dirname,
                                                        fig_index),
                            bbox_inches='tight')
                fig_index += 1
                plt.close(fig)
...
class GANTwoLayers(object):
    def __init__(self, noise_dim, input_dim,
                 G_hidden_layer, D_hidden_layer,
                 init_lr=0.001, decay_steps=10000,
                 optimizer=tf.train.AdamOptimizer, name="Vanilla2LGAN"):
        self.noise_dim = noise_dim
        self.input_dim = input_dim
        self.G_hidden_layer = G_hidden_layer
        self.D_hidden_layer = D_hidden_layer
    ...
    def _create_variables(self):
        ...
        G_W2 = tf.Variable(self.xavier_init([self.G_hidden_layer[0],
                                             self.G_hidden_layer[1]]))
        G_b2 = tf.Variable(tf.zeros(shape=[self.G_hidden_layer[1]]))

        G_W3 = tf.Variable(self.xavier_init([self.G_hidden_layer[1],
                                             self.input_dim]))
        ...
        D_W2 = tf.Variable(self.xavier_init([self.D_hidden_layer[0],
                                             self.D_hidden_layer[1]]))
        D_b2 = tf.Variable(tf.zeros(shape=[self.D_hidden_layer[1]]))

        D_W3 = tf.Variable(self.xavier_init([self.D_hidden_layer[1], 1]))
    ...
    def discriminator(self, x, keep_prob=0.6):
        D_W1, D_b1 = self.theta_D['D_W1'], self.theta_D['D_b1']
        D_W2, D_b2 = self.theta_D['D_W2'], self.theta_D['D_b2']
