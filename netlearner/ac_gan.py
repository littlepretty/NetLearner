...
class AuxiliaryClassifierGAN(object):
    def __init__(self, noise_dim, input_dim, label_dim,
                 ...
                 init_lr, decay_steps,
                 optimizer=tf.train.AdamOptimizer, name='ACGAN'):
        ...
        self.Y = tf.placeholder(tf.float32, [None, label_dim], name='label')
        ...
        self.L_neg = tf.reduce_mean(-tf.log(D_real) - tf.log(1.0 - D_fake))
        self.C_neg = self.cross_entropy(C_real_logits, self.Y) + \
            self.cross_entropy(C_fake_logits, self.Y)
        self.D_V_neg = self.L_neg + self.C_neg
        ...
        self.sess.run(init)
        print('AC-GAN build and initialized')
    ...
    def _create_summaries(self):
        ...
        tf.summary.scalar('L', -self.L_neg)
    ...
    def make_summary(self, step, X, Z, Y):
    ...
    def train(self, batch_size, train_dataset, train_labels, num_steps):
        ...
        print(display_Y.shape, display_Z.shape)
        ...
        for step in xrange(num_steps):
            ...
            if step % summary_step == 0:
                ...
                self.make_summary(step, batch_X, batch_Z_G, batch_Y)

        Z_D = self.sample_noise([X.shape[0], self.noise_dim])
        D_V_neg = self.sess.run(self.D_V_neg, feed_dict={self.X: X,
                                                         self.Y: Y,
                                                         self.Z: Z_D})
        ...
        self.make_summary(num_steps, X, Z_G, Y)
