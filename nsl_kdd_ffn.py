...
train_dataset = np.load('NSL-KDD/train_dataset.npy')
train_labels = np.load('NSL-KDD/train_ref.npy')
valid_dataset = np.load('NSL-KDD/valid_dataset.npy')
valid_labels = np.load('NSL-KDD/valid_ref.npy')
test_dataset = np.load('NSL-KDD/test_dataset.npy')
...
with graph.as_default():
    ...
    W1 = tf.Variable(tf.truncated_normal([feature_size, hidden_units1],
                                         stddev=0.1))
    ...
    W4 = tf.Variable(tf.truncated_normal([hidden_units3, num_labels],
                                         stddev=0.12))
    ...
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(model, tf_train_labels))
    ...
    learning_rate = tf.train.exponential_decay(init_learning_rate, global_step,
                                               ...
                                               staircase=True)
    # notice that here optimizer will help us increment global step
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        if step % 10 == 0:
            ...
            print("Minibatch train accuracy: %f%%" %
                  accuracy(train_predictions, batch_labels))

            feed_dict = {tf_valid_dataset: valid_dataset,
                         keep_prob: 1.0}
            print("Minibatch validation accuracy: %f%%" %
                  accuracy(valid_predict.eval(feed_dict=feed_dict),
                           valid_labels))
    ...
    feed_dict_valid = {tf_valid_dataset: valid_dataset, keep_prob: 1.0}
...
def measure_prediction(predictions, labels, dataset_name='Test'):
    print("%sset accuracy: %f%%" %
          (dataset_name, accuracy(predictions, labels)))
...
measure_prediction(valid_predictions, valid_labels, 'Validation')
