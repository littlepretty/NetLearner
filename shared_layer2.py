...
from keras.layers import Dense, Input, concatenate, Flatten, Dropout, Conv2D
...
def shared_model(unsw_h1, nsl_h1, reg_beta=0.001):
    dropout = Dropout(0.2)
    ...
    h2 = Dense(400, activation='relu', name='h2',
               kernel_regularizer=regularizers.l2(reg_beta))(bn)

    return h2


def m_model(unsw_merge, nsl_merge, unsw_inputs, nsl_inputs, reg_beta=0.001):
    unsw_h1 = Dense(800, activation='relu', name='h1',
                    kernel_regularizer=regularizers.l2(reg_beta))(unsw_merge)

    nsl_h1 = Dense(800, activation='relu', name='h1',
                   kernel_regularizer=regularizers.l2(reg_beta))(nsl_merge)

    h2 = shared_model(unsw_merge, nsl_merge)
    unsw_h1 = Dense(800, activation='relu', name='unsw_h1',
                    # kernel_regularizer=regularizers.l2(reg_beta)
                    )(unsw_merge)

    nsl_h1 = Dense(800, activation='relu', name='nsl_h1',
                   # kernel_regularizer=regularizers.l2(reg_beta)
                   )(nsl_merge)
    unsw_dropout = Dropout(0.2)(unsw_h1)
    nsl_dropout = Dropout(0.2)(nsl_h1)

    unsw_h3 = Dense(400, activation='sigmoid', name='unsw_h3',
               # kernel_regularizer=regularizers.l2(reg_beta)
               )(unsw_dropout)

    nsl_h3 = Dense(400, activation='sigmoid', name='nsl_h3',
               # kernel_regularizer=regularizers.l2(reg_beta)
               )(nsl_dropout)

    unsw_sm = Dense(2, activation='softmax', name='unsw_output')(unsw_h3)

    nsl_sm = Dense(2, activation='softmax', name='nsl_output')(nsl_h3)

    unsw_model = Model(inputs=unsw_inputs, outputs=unsw_sm)
    unsw_model.compile(optimizer='adam', loss='binary_crossentropy',
                       metrics=['accuracy'])

    nsl_model = Model(inputs=nsl_inputs, output=nsl_sm)
...
def master_model(unsw_merge, nsl_merge, unsw_inputs, nsl_inputs, reg_beta=0.001):
    '''h1 = Dense(800, activation='relu', name='h1',
               kernel_regularizer=regularizers.l2(reg_beta))

    unsw_h1 = h1(unsw_merged)

    nsl_h1 = h1(nsl_merged)'''
    ...
    unsw_d = Dropout(0.2)(unsw_h1)
    nsl_d = Dropout(0.2)(nsl_h1)

    #unsw_bn = BatchNormalization(name='unsw_bn_1')(unsw_d)
    #nsl_bn = BatchNormalization(name='nsl_bn_1')(nsl_d)

    h2 = Dense(800, activation='relu', name='h1',
          # kernel_regularizer=regularizers.l2(reg_beta)
          )
    ...
    #unsw_sm = Dense(2, activation='softmax', name='unsw_output')
    #nsl_sm = Dense(2, activation='softmax', name='nsl_output')
    sm = Dense(2, activation='softmax', name='output')

    #unsw_model = Model(inputs=unsw_inputs, outputs=sm)
    unsw_model = Model(inputs=unsw_inputs, outputs=sm(unsw_h3(unsw_h2)))
    unsw_model.compile(optimizer='adam', loss='binary_crossentropy',
                  metrics=['accuracy'])

    #nsl_model = Model(inputs=nsl_inputs, output=sm)
    nsl_model = Model(inputs=nsl_inputs, output=sm(nsl_h3(nsl_h2)))
    nsl_model.compile(optimizer='adam', loss='binary_crossentropy',
                  metrics=['accuracy'])
...
#m1, m2 = m_model(unsw_tens, nsl_tens, unsw_inputs, nsl_inputs)
...
score = m1.evaluate(unsw_test, unsw_test_y, unsw_test_y.shape[0], verbose=1)
...
history = m1.fit(unsw_train, unsw_train_y,
                 epochs=10, batch_size=batch_size, shuffle=True,
                 steps_per_epoch=None, callbacks=[csv_logger],
                 validation_data=(unsw_test, unsw_test_y))
